...
from utils.utils import cosine_similarity, cosine_similarity_individual
from vit_image_experiments.data_handler import get_dataloaders
from vit_image_experiments.model import VitModel

class CFG:
    model_name = "vit_multilabel"
    input_size = 224
    batch_size = 64
    num_epochs = 5
    lr = 1e-4
...
def train_vit_images():
    df = pd.read_csv(os.path.join(root_dir, csv_name), low_memory=False)
    train_dataloader, val_dataloader = get_dataloaders(df, CFG)
    ...
    for epoch in range(CFG.num_epochs):
        ...
        for X, y, classes, weight in pbar:
