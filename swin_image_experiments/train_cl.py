...
class CFG:
    model_name = "swin_images_multi_label_cl"
    input_size = 256
    batch_size = 64
    num_epochs = 4
    lr = 1e-4
...
def train_swin_images_cl():
    df = pd.read_csv(os.path.join(root_dir, csv_name))
    trn_df, _ = train_test_split(df, test_size=0.1, random_state=CFG.seed)
    cos_errors = joblib.load("cos_errors.joblib")
    weighted_cos_error = {k: np.average(v, weights=[ 0.1, 0.2, 0.7]) for k, v in cos_errors.items()}

    temp_df = pd.DataFrame.from_dict(
        {'paths': list(weighted_cos_error.keys()), 'cos': list(weighted_cos_error.values())})
    new_train_df = pd.merge(left=trn_df, right=temp_df, how='inner', left_on='filepath',
                            right_on='paths')
    ...
    ttl_iters = (CFG.num_epochs-1) * len(train_dataloader)
